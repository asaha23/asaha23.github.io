...
/* actions ---------------------------------------------*/
  ...
  void performAction() {
    ...
    // timer buttons
    else if   (actionName.equals("toggleTimer"))              {
			toggleTimer();
			if (type.equals("play")) {type = "pause";}
			else {type = "play";}
		}
		...
		// for molecule adding buttons
		for (int i = 0; i < allSpecies.length; i++) {
			String speciesName = allSpecies[i];
			if (actionName.equals("add 5 "+ speciesName)) {
				createMolecule(speciesName, 5);
			}
		}
